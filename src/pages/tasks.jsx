...
import { getLocale, formatSafeDate } from "@/components/utils/i18n-utils";
...
export default function TasksPage() {
  ...
  const [filters, setFilters] = useState(() =>
    loadFromStorage("tasksPage_filters", initialFilters),
  );
  const [sortConfig, setSortConfig] = useState(() =>
    loadFromStorage("tasksPage_sortConfig", {
      key: "created_date",
  ...
  const [currentView, setCurrentView] = useState(() => {
    try {
      ...
      console.warn(
        'Failed to load view preference from storage, defaulting to "card":',
        e,
      );
      return "card";
    }
    ...
    const sorted = [...filteredTasks].sort((a, b) => {
      ...
      if (aValue && bValue) {
        if (typeof aValue === "string" && typeof bValue === "string") {
          ...
          isValid(parseISO(String(aValue))) &&
          isValid(parseISO(String(bValue)))
        ) {
          // Handle date strings robustly
          comparison =
            parseISO(String(aValue)).getTime() -
            parseISO(String(bValue)).getTime();
        } else {
    ...
    async (taskData) => {
      try {
        ...
        if (currentTaskForDialog && currentTaskForDialog.id) {
          ...
          toast({
            ...
            description: t("tasks.updateSuccess", {
              name: taskData.title || t("common.item"),
            }),
          });
        } else {
        ...
        if (isSelectionModeActive) {
          // Ensure this is called after successful save & dialog close
        ...
        toast({
          ...
          title: t("common.error", { defaultValue: "Error" }),
          description:
            t("common.saveError", { defaultValue: "Failed to save task." }) +
            ` ${error.message}`,
        });
  ...
  const handleDeleteTasks = useCallback(
    async (taskIds) => {
      ...
      const itemsToDelete = allTasks.filter((task) =>
      ...
      const itemName =
        itemsToDelete.length === 1
          ? firstItemName
          : t("pageTitles.tasksMultipleItems", {
              count: taskIds.length,
              defaultValue: `${taskIds.length} tasks`,
            });
  ...
  const confirmDelete = useCallback(async () => {
    if (!deleteDialogState.itemIds || deleteDialogState.itemIds.length === 0)
      return;
    setLoading(true);
    let successCount = 0;
    let failedCount = 0;
    ...
    for (const id of deleteDialogState.itemIds) {
      try {
        ...
        errorsEncountered.push(
          t("tasks.deleteError", {
            name: allTasks.find((t) => t.id === id)?.title || id,
            error: err.message,
          }),
        );
      }
    ...
    if (successCount > 0) {
      toast({
        title: t("messages.success"),
        description: t("tasks.bulkDeleteSuccess", { count: successCount }),
      });
    }
    if (failedCount > 0) {
      toast({
    ...
    if (isSelectionModeActive) {
      // Ensure this is called after successful delete
      handleCancelSelectionMode();
    }
  }, [
    deleteDialogState,
  ...
  const handleStartSelectionMode = useCallback(
    (mode, task = null) => {
      ...
      toast({
        title: t("bulkActions.selectionModeActiveTitle", {
          mode: t(`common.${mode.toLowerCase()}`, { defaultValue: mode }),
        }),
        description: t("bulkActions.selectionModeActiveDesc", {
          mode: t(`common.${mode.toLowerCase()}`, { defaultValue: mode }),
          entity: t("pageTitles.tasksPlural"),
        }),
        variant: "info",
      });
    },
  ...
  const paginatedTasks = useMemo(() => {
    ...
    const startIndex =
      (cardPagination.currentPage - 1) * cardPagination.pageSize;
  ...
  const totalCardPages = Math.ceil(
    (filteredAndSortedTasks?.length || 0) / cardPagination.pageSize,
  );
  ...
  const handleImportSubmit = async (records) => {
    ...
    if (!records || records.length === 0) {
      toast({
        title: t("import.noRecordsTitle"),
        description: t("import.noRecordsDesc"),
        variant: "warning",
      });
      return;
    }

    const tasksToCreate = records
      .map((rec) => ({
    ...
    if (tasksToCreate.length === 0) {
      toast({
        title: t("import.noValidRecordsTitle"),
        description: t("import.noValidRecordsDesc", {
          entity: t("pageTitles.tasks"),
        }),
        variant: "warning",
      });
      return;
    ...
    for (const taskData of tasksToCreate) {
      try {
        ...
        toast({
          ...
          description: t("errors.importTaskFailedDesc", {
    ...
    toast({
      ...
      description: t("import.completedDesc", {
        successCount,
        errorCount: tasksToCreate.length - successCount,
        entity: t("pageTitles.tasks"),
      }),
    });
  ...
  const allVisibleSelected = useMemo(() => {
    if (currentView === "card") {
      return (
        ...
        paginatedTasks.every((task) => selectedItems.includes(task.id))
      );
    }
  ...
  const allTableSelected = useMemo(() => {
    return (
      (filteredAndSortedTasks?.length || 0) > 0 &&
      filteredAndSortedTasks.every((task) => selectedItems.includes(task.id))
    );
      ...
      <PageLayout>
        <PageHeader
          ...
          description={t("tasks.pageDescription", {
            defaultValue: "Manage your team's tasks and projects.",
          })}
          icon={PageIcon}
        />
  ...
  const noTasksAfterFiltering =
    !loading &&
    (!filteredAndSortedTasks || filteredAndSortedTasks.length === 0) &&
    hasActiveFilters;
  const noTasksAtAll =
    !loading && (!allTasks || allTasks.length === 0) && !hasActiveFilters;
    ...
    <PageLayout>
      ...
      <div className="mb-4 flex flex-col sm:flex-row justify-between items-center gap-2">
            ...
            <div className="flex items-center gap-2 p-2 bg-blue-50 border border-blue-200 rounded-md shadow-sm">
              <span className="text-sm font-medium text-blue-700">
                {selectedItems.length > 0
                  ...
                  : t("bulkActions.selectItemsPromptShort", {
                      mode: t(
                        `common.${selectionMode?.toLowerCase() || "action"}`,
                      ),
                    })}
              </span>
              <Button
        ...
        <div className="flex items-center gap-2">
          ...
          <GlobalActionDropdown
            ...
            onEdit={() => {
              if (selectedItems.length === 1) {
                const taskToEdit = allTasks.find(
                  (t) => t.id === selectedItems[0],
                );
                if (taskToEdit) handleOpenTaskDialog(taskToEdit);
              } else {
                toast({
                  title: t("bulkActions.selectOneToEditTitle", {
                    entity: t("pageTitles.task"),
                  }),
                  description: t("bulkActions.selectOneToEditDesc", {
                    entity: t("pageTitles.task"),
                  }),
                  variant: "info",
                });
            ...
            onDeleteItems={() => {
              if (selectedItems.length > 0) {
                ...
                toast({
                  ...
                  description: t("bulkActions.selectItemsPrompt", {
                    mode: t("common.delete"),
                  }),
                  variant: "info",
                });
              }
            ...
            onExport={() => {
              toast({
                ...
                description: t("common.featureComingSoonDesc", {
                  featureName: t("common.export"),
                }),
              });
            }}
            onAssign={() => {
              toast({
                title: t("common.featureComingSoonTitle"),
                description: t("common.featureComingSoonDesc", {
                  featureName: t("common.assignTasks"),
                }),
              });
            }}
          ...
          >
            {loading ? (
              <LoadingSpinner size={16} message={null} />
            ) : (
      ...
      {loading &&
        (!filteredAndSortedTasks || filteredAndSortedTasks.length === 0) && ( // Adjusted condition
          <LoadingSpinner
            className="mt-20"
            message={t("messages.loadingData", {
              item: t("pageTitles.tasksPlural", { defaultValue: "Tasks" }),
            })}
          />
        )}
      ...
      {error &&
        allTasks &&
        allTasks.length > 0 && ( // Show error even if some tasks loaded
      ...
      {!loading && (noTasksAtAll || noTasksAfterFiltering) ? (
        <>
          {noTasksAtAll && (
            <EmptyState
              icon={ListTodo}
              title={t("emptyStates.noTasksTitle", {
                defaultValue: "No Tasks Yet",
              })}
              description={t("emptyStates.noTasksMessage", {
                defaultValue: "Get started by creating a new task.",
              })}
              actionButton={
          ...
          {noTasksAfterFiltering && (
            <EmptyState
              icon={Filter}
              title={t("emptyStates.noTasksFilterTitle", {
                defaultValue: "No Tasks Match Filters",
              })}
              description={t("emptyStates.noTasksFilterMessage", {
        ...
        <div className="space-y-4">
          {" "}
          {/* Added wrapper div as per outline */}
          ...
          {currentView === "card" && TaskList && (
            <TaskList
              ...
              onStatusChange={async (task, newStatus) => {
                // Adding onStatusChange handler for TaskList
                try {
                  await Task.update(task.id, { ...task, status: newStatus });
                  toast({
                    title: t("messages.success"),
                    description: t("tasks.statusUpdateSuccess", {
                      name: task.title,
                  ...
                  toast({
                    ...
                    description: t("tasks.statusUpdateError", {
                      name: task.title,
                      error: err.message,
                    }),
                  });
          ...
          {currentView === "table" && DataTable && (
            <DataTable
              columns={[
                ...
                },
                {
                  ...
                  cell: ({ row }) => (
                    <Badge
                      variant={
                        row.original.status === "done" ? "default" : "outline"
                      }
                    >
                ...
                },
                {
                  ...
                  header: t("tasks.fields.category", {
                ...
                },
                {
                  ...
                  cell: ({ row }) =>
                    row.original.due_date
                      ? formatSafeDate(row.original.due_date, currentLocale, {
                          dateStyle: "medium",
                        })
                      : "-",
                  enableSorting: true,
                },
                {
                  ...
                  header: t("tasks.fields.assignedToShort", {
                ...
                },
                {
                  ...
                  header: t("common.createdDate", { defaultValue: "Created" }),
                  cell: ({ row }) =>
                  ...
                  cell: ({ row }) => (
                    <div className="flex gap-1">
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleOpenTaskDialog(row.original);
                        }}
                      >
                      ...
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleDeleteTasks([row.original.id]);
                        }}
                      >
              ...
              onRowClick={({ original: task }) => {
                // Handle row click for details or edit
                if (isSelectionModeActive) {
              ...
              onSelectAllRows={() => {
                // Handle select all rows for DataTable
                if (allTableSelected) {
                  setSelectedItems([]);
                } else {
              ...
              currentSort={[
                {
                  id: sortConfig.key,
                  desc: sortConfig.direction === "descending",
                },
              ]}
          ...
          {currentView === "kanban" && KanbanBoard && (
            <KanbanBoard
              ...
              onAddTask={(status) =>
                handleOpenTaskDialog({ status: status || "todo" })
              }
              t={t}
              language={language}
              isRTL={isRTL}
      ...
      <ConfirmationDialog
        ...
        title={t("common.confirmDeleteTitleShort", {
          item: deleteDialogState.itemName,
          defaultValue: `Delete ${deleteDialogState.itemName}`,
        })}
