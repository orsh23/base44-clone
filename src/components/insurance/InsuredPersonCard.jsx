...
import { User, Mail, Phone, Calendar, IdCard, MapPin } from 'lucide-react';
...
const InsuredPersonCard = ({
  ...
  const {
    ...
    gender,
    contact,
    address, // Assuming legacy address string for simplicity here
    identification,
    updated_date,
  } = person;
  ...
  const idTypeDisplay = identification?.type ? t(`idType.${identification.type.toLowerCase()}`, {defaultValue: identification.type}) : t('common.notSet');
  const genderDisplay = gender ? t(`gender.${gender.toLowerCase()}`, {defaultValue: gender}) : t('common.notSet');

  const renderField = (IconComponent, value, fallback = t('common.notSet', {defaultValue: 'N/A'})) => {
    if (!value && value !== 0) return null; // Don't render if value is not present
    return (
      ...
      {isSelectionModeActive && (
        <div className={`absolute top-2 ${isRTL ? 'left-2' : 'right-2'} z-10`}>
          <Checkbox
            ...
            aria-label={t('bulkActions.selectItem', {item: full_name || t('insurance.insuredPersons.singularTitle', {defaultValue: 'Insured Person'})})}
      ...
      <CardHeader className={`pb-2 ${isSelectionModeActive ? (isRTL ? 'pr-10' : 'pl-10') : ''}`}>
        <div className="flex justify-between items-start">
            <CardTitle className="text-md font-semibold text-gray-800 dark:text-gray-100 group-hover:text-blue-600 dark:group-hover:text-blue-500 transition-colors flex items-center">
                <User className={`h-5 w-5 ${isRTL ? 'ml-2' : 'mr-2'} text-blue-600 dark:text-blue-400`} />
                <span className="truncate" title={full_name}>{full_name || t('common.unknown')}</span>
            </CardTitle>
        </div>
         <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
            {identification?.number && <Badge variant="outline" className="text-xs truncate" title={`${idTypeDisplay}: ${identification.number}`}>{`${idTypeDisplay}: ${identification.number}`}</Badge>}
            {gender && <Badge variant="outline" className="text-xs">{genderDisplay}</Badge>}
        </div>
      </CardHeader>

      <CardContent className="text-sm flex-grow py-2 px-4 space-y-1">
        {renderField(Calendar, formatDate(date_of_birth))}
        {renderField(Mail, contact?.email)}
        {renderField(Phone, contact?.phone)}
        {address?.city && renderField(MapPin, `${address.street_name || ''} ${address.street_number || ''}, ${address.city}`.replace(/^ , |, $/g, '').replace(/, ,/g, ','))} 
      </CardContent>
...
export default InsuredPersonCard;
