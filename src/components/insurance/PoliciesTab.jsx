...
export default function PoliciesTab({ globalActionsConfig: externalActionsConfig, currentView: passedView }) {
  ...
  const {
    items: policies, // Renamed from 'items' for clarity (all policies before filtering)
    loading,
    error,
    filters,
    setFilters,
    ...
    setSortConfig,
    pagination,
    selectedItems, // Now an array by default from useEntityModule
    setSelectedItems,
    isDialogOpen,
    setIsDialogOpen,
    currentItem,
    ...
    filteredAndSortedItems, // This is the array we need to be careful with for rendering
  ...
  const handleEditWithSelectionCheck = useCallback(() => {
    if (selectedItems.length === 1) {
      const policyIdToEdit = selectedItems[0];
      // Ensure filteredAndSortedItems is an array before using .find()
      const policyToEdit = Array.isArray(filteredAndSortedItems) ?
        filteredAndSortedItems.find(p => p.id === policyIdToEdit) : null;
      if (policyToEdit) {
  ...
  const handleDeleteWithSelectionCheck = useCallback(() => {
    if (selectedItems.length > 0) {
      const idsToDelete = selectedItems; // selectedItems is already an array
      // Ensure filteredAndSortedItems is an array
      const firstItemName = idsToDelete.length > 0 && Array.isArray(filteredAndSortedItems) ?
        (filteredAndSortedItems.find(p => p.id === idsToDelete[0])?.policy_number || t('policies.itemTitleSingular')) : t('policies.itemTitleSingular');
  ...
  const handleConfirmDelete = async () => {
    ...
    if (!deleteDialogState.itemIds || !Array.isArray(deleteDialogState.itemIds) || deleteDialogState.itemIds.length === 0) return;
    
    // Close dialog before calling handleBulkDelete
    setDeleteDialogState(prev => ({ ...prev, isOpen: false })); 

    const { successCount, errorCount } = await handleBulkDelete(deleteDialogState.itemIds);
  ...
  const handleImportSubmit = async (records) => {
    ...
    if (!Array.isArray(records) || records.length === 0) { // Safe access
    ...
    const policiesToCreate = records.map(rec => ({
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-2 sticky top-[calc(var(--header-height,0px)+var(--subheader-height,0px))] bg-background dark:bg-gray-900 py-3 z-10 -mx-1 px-1 md:mx-0 md:px-0 border-b dark:border-gray-700">
          ...
          <div className="flex items-center gap-2">
              <GlobalActionButton
                  ...
                  selectedItemCount={selectedItems.length} // Safe access
      ...
      {!loading && !error && safeFilteredAndSortedItems.length === 0 && (
        <EmptyState
          icon={Shield}
          title={
            Object.values(filters).some(v => v && v !== 'all' && v !== '' && v !== entityConfig.initialFilters.pageSize)
        ...
        <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {safeFilteredAndSortedItems.map(policy => (
                <PolicyCard
                  ...
                  t={t} isRTL={isRTL}
                  isSelectionModeActive={isSelectionModeActive}
                  isSelected={selectedItems.includes(policy.id)} // Check if ID is in the array
          ...
          {currentView === 'table' && (
            <DataTable
              ...
              isSelectionModeActive={isSelectionModeActive}
              selectedRowIds={new Set(selectedItems)} // DataTable expects a Set for selected IDs
              onRowSelectionChange={(rowId, isSelected) => handleToggleSelection(rowId)} // use toggle handler directly
      ...
      <ConfirmationDialog
        ...
        title={t('common.confirmDeleteTitle', {item: deleteDialogState.itemName || t('policies.itemTitleSingular'), count: deleteDialogState.itemIds?.length || 1})}
